...
#        pixel_ring.set_color(rgb=0x400000)
    ...
    def _callback(self, in_data, frame_count, time_info, status):
        ...
        if self.status & self.listening_mask:
            ...
            if active:
                if not self.active:
                    for d in self.listen_history:
                        ...
                        self.listen_countdown[1] = self.end_silence
            ...
            if (self.listen_countdown[1] < -(self.end_silence) and self.listen_countdown[0] < (1/10)*self.end_silence) \
                or (self.listen_countdown[0] <= 0) \
                or (self.listen_countdown[1] <= 0):            
